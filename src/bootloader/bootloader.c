 ...
 *
 * Boot loader.
 * Part of the boot sector, along with bootasm.S, which calls load().
...
void read_segment(uchar*, uint, uint);

// load kernel image from 1 sector, try to load 1M
void load(void) {
	struct elfhdr *elf;
...
void read_segment(uchar* pa, uint count, uint offset) {
	uchar* epa;
